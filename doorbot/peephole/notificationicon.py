...
# modified for python3 by indigoparadox
...
logger = logging.getLogger( 'notificationicon' )
...
class NotificationIcon(object):
    def __init__(self, iconfilename, tooltip=None):
        ...
        self._tooltip = tooltip if tooltip else u''
    ...
    def _run(self):
        ...
        while not self._die:
            try:
                ...
                logger.error( "NotificationIcon error: {}: {}".format(
                    err, message ) )
                message = MSG()
            time.sleep(0.125)
        logger.error( "Icon thread stopped, removing icon..." )
    ...
    def die(self):
        ...
        try:
            ...
            logger.error( "Icon remove error: {}".format( err ) )
        ctypes.windll.user32.DestroyWindow(self._hwnd)
        ctypes.windll.user32.DestroyIcon(self._hicon)
